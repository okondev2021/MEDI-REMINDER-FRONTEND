...
const MedicationHistory = () => {
    ...
    const [medicationHistory, setMedicationHistory] =
        useState<GroupedDosesProps[]>();

    const getBriefMedicationHistory = async () => {
        const currentLocalTime =
            DateTime.now().setZone(userProfileInfo?.timezone).endOf("day") ??
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            {/* Header */}
            <div className="flex flex-col gap-y-3 mb-6 md:flex-row md:items-center md:justify-between">
            ...
            <div className="space-y-8">
                {!medicationHistory && (
                    <LoadingSpinner size="lg" label="History Loading" />
                )}

                {medicationHistory && medicationHistory.length < 1 && (
                    <div className="text-center py-6">
                        <p className="text-sm text-gray-500">
                            You have no medication doses yet. Add one to get started.
                        </p>
                    </div>
                )}
                    ...
                    <div key={day.time}>
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            <span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm font-medium">
                                {day.time === now.toFormat("yyyy-MM-dd")
                                    ? "Today"
                                    : day.time === yesterday
                                        ? "Yesterday"
                                        : day.time}
                            ...
                            {day.medications.map((med, index) => (
                                <div key={med.id} className="relative pl-8 pb-6 last:pb-0">
                                    ...
                                    <div className="absolute left-0 top-0 h-full flex flex-col items-center">
                                        <div
                                            className={`w-3 h-3 rounded-full border-2 ${med.taken === true
                                                    ? "border-green-500 bg-green-100"
                                                    : DateTime.fromMillis(med.Timestamp.toMillis()).toLocal() <
                                                        now || med.missed === true
                                                        ? "border-red-500 bg-red-100"
                                                        : "border-yellow-500 bg-yellow-100"
                                                }`}
                                        ></div>
                                        {index + 1 !== day.medications.length && (
